...
const OrganizerMobileNavigation = () => {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-50 backdrop-blur-sm lg:hidden border-t-2"
      style={{
        backgroundColor: "hsl(var(--org-bg))",
        borderTopColor: "hsl(var(--org-accent))",
      }}
    >
        ...
        {items.map(({ icon: Icon, label, path }) => {
          ...
          return (
            <Link
              ...
              to={path}
              className={cn(
                "flex flex-col items-center p-2 sm:p-3 min-w-0 transition-all duration-300",
